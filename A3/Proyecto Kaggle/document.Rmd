...
Para decidir que modelo vamos a utilizar, primero probamos diferentes modelos que hemos ido viendo durante la asignatura, usando una de las técnicas vistas en las diferentes actividades. Así que, se utilizó una validación cruzada de 5 carpetas (5cv) sobre el conjunto de datos de entrenamiento. Para quedarnos con el mejor modelo se usaron las matrizes de confusión de cada predicción realizada por los modelos y se tubo en cuenta los valores de la métrica *Accuracy*. A continuación, podemos observar una tabla con los valores de *Accuracy* de los modelos que se han probado. 
...
Como se observa en la tabla anterior, el mejor modelo es un modelo lineal generalizado con un valor de *Accuracy* de 0.9294118. Concretamente, se trata del modelo binominal de regressión. Este tipo de modelo ha funcionado tant bien porqué el problema que se nos ha planteado presenta unos inputs con sus outputs deseados, y el objetivo es aprender una regla general que asigne las entradas a las salidas. Además en un modelo de regressión los outputs son numéricos, no categoricos como en clasificación. También contamos con que sea un aprendizaje supervisado ya que la variable clase nos da información.

Hemos escogido el modelo binominal ya que la variable a predecir es binaria.
