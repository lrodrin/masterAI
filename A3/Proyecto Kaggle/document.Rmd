...
Una vez dividido el conjunto de los datos, es necesario realizar un análisis del conjunto de datos de entrenamiento, así como un proceso de exploración, transformación y limpieza de esos datos con el objetivo de resaltar información útil para la fase de modelado. Este análisis nos permitirá controlar la presencia de valores fuera de rango, una idea inicial de la forma que tienen los datos, etc. así como las relaciones entre los distintos atributos. Aunque para sintetizar solo se comentará aquello que se ha considerado más interesante durante el análisis.
...
Empezamos analizando visualmente la variable de clase a predecir (atributo *"shot_made_flag"*) que es binaria y que se distribuye de manera bastante equitativa, vemos que el número de canastas que no entraron en el aro es superior al número de canastas que sí que entraron en el aro. Así que, intentaremos ver si esto puede estar relacionado con la gran lesión que tuvo durante la temporada 2013-14.
...
A continuación, analizamos visualmente la precisión de los lanzamientos realizados por la temporada (atributo *"season"*), y vemos que a partir de la temporada 2013-14 la precisión de los lanzamientos baja drásticamente. Así que, una gran cantidad de los lanzamientos que no entraron en el aro están correlacionados a la gran lesión que tuvo en la temporada 2013-14.
...
En el siguiente gráfico analizamos visualmente la precisión de los lanzamientos por la distancia (atributo *"shot_distance"*), porqué en diferentes exploraciones de datos que han realizado otros usuarios en *Kaggle*, se ha podido observar que el atributo *"shot_distance"* contiene valores fuera de rango que podríamos eliminar en el apartado de limpieza de datos y que nos sería muy beneficioso para reducir los fallos durante la predicción en base a la distancia de los lanzamientos. Los valores fuera de rango podrían encontrarse a partir de los lanzamientos realizados a más de 30 (ft.) ya que la precisión de estos baja drásticamente.
...
Y en siguiente grafico podemos observar de su existencia en el conjunto de datos de entrenamiento. Y que valores exactamente se encuentran fuera de rango (a partir de 40 (ft.)). Así que, en el apartado de limpieza de datos serán eliminados los atributos con *shot_distance* superiores a 40 (ft.).
...
Anteriormente, hemos analizado visualmente la precisión de los lanzamientos por temporada, los lanzamientos por distancia y hemos encontrado valores fuera de rango en el conjunto de datos que tendremos que eliminar si queremos realizar una buena predicción de la variable clase *"shot_made_flag"*.
...
Es importante hacer esta transformación porqué después de realizar el análisis del conjunto de datos, probar diferentes modelos y explorar algunas soluciones encontradas en Kaggle se ha considerado que la predicción se realizará sobre un conjunto de datos aún más reducido que va a tener como atributos (*"time_remaining"* y *"shot_distance"*) y la variable clase *"shot_made_flag"*. Para predecir durante el tiempo que duraron los lanzamientos acertados según la distancia del lanzamiento. Para ello también será necesaria la normalización de los atributos (*"time_remaining"* y *"shot_distance"*).
...
Después de la limpieza los conjuntos de datos de entrenamiento y de test son:
...
Para decidir el modelo final probamos diferentes modelos que hemos ido viendo durante la asignatura sobre el conjunto de datos de entrenamiento. Para quedarnos con el mejor modelo usamos las matrices de confusión de cada modelo resultantes de las predicciones sobre el conjunto de datos de entrenamiento realizadas para comparar los valores de la métrica *Accuracy* entre ellos. A continuación, podemos observar una tabla con los valores de *Accuracy* de los modelos que se han probado. 

```{r message=FALSE, warning=FALSE, echo=FALSE}
table <- matrix(c(0.5972292, 0.6083590, 0.6099156, 0.6085924, 0.7203565, 0.5879675, 0.6076585, 0.8043351, 0.9294118), ncol=1,byrow=TRUE)
...
Como se observa en la tabla anterior, el mejor modelo es el modelo lineal generalizado con un valor de *Accuracy* de 0.9294118. Concretamente, es el modelo binominal de regresión. Dentro de los diferentes tipos de modelos lineales se escogió el modelo binominal porqué la variable a predecir es binaria.

Este tipo de modelo ha funcionado tan bien porqué el problema que se nos ha planteado tiene como objetivo aprender qué valores de las entradas del conjunto de datos de entrenamiento asignamos a los valores de salida del conjunto de datos de test. Además, esperamos como salida valores numéricos, esperados en un modelo de regresión, y no valores categóricos como los esperados en una clasificación.
...
En la tabla coeficientes, tanto *shot_distance* como *time_remaining* son atributos altamente significativos, con p-valores bajísimos, lo que demuestra que ambos atributos son importantes para predecir la variable dependiente (*shot_made_flag*).

# **Resultado**

Finalmente, predecimos sobre el conjunto de datos de test y guardamos el resultado en un fichero *.CSV" para la presentación del resultado en Kaggle. A continuación, vemos los primeros valores del resultado obtenido para tener una idea de como es el fichero que se ha subido.
...
El hecho de que un modelo con el que aún no habia trabajado durante la asignatura fuera el que me dió el mejor resultado me dió la posibilidad de aprender más cosas. Sobretodo al hacer una evaluación del modelo. Que he encontrado difícil de realizar. Creo que aún no lo he entendido al 100%. Después me costó mucho correlacionar y ver que atributos seleccionar para la predicción. 
