...
*Accuracy representa el porcentaje total de elementos clasificados correctamente. Es la medida más directa de la calidad de los clasificadores. Es un valor entre 0 y 1. Cuanto más alto, mejor.*
...
El *Accuracy* promedio estimado mediante validación cruzada repetida es de 0.7650943, el modelo predice correctamente un 76.5% de las veces.
...
Empleando un modelo Random Forest con mtry = 7 se consigue un *Accuracy* promedio de validación igual a 0.7618528, el modelo predice correctamente un 76.2% de las veces.
...
Empleando un modelo Boosting con n.trees = 100, interaction.depth = 2, shrinkage
 = 0.1 and n.minobsinnode = 2 se consigue un *Accuracy* promedio de validación igual a 0.7451281, el modelo predice correctamente un 74.5% de las veces.
...
Empleando un modelo SVM Radial con sigma = 0.006839614 y C = 1, se consigue un *Accuracy* promedio de validación igual a 0.7701726, el modelo predice correctamente un 77% de las veces.
...
Las métricas obtenidas mediante validación cruzada son estimaciones de la capacidad que tiene un modelo al predecir nuevas observaciones.
...
summary(resamps)
```
Observaras que junto
con la estimacion del porcentaje de bien clasicados (\accuracy"), se nos muestra el valor del estadstico Kappa,
el cual es una medida que compara el \accuracy" observado respecto al \accuracy" esperado (de un predictor
al azar). Esto es, cuanto mejor estimamos que lo hace nuestro clasicador respecto a otro que predijese al azar
la clase (siguiendo la probabilidad a priori de estas). Un ameno ejemplo para entenderla mejor se encuentra en
aqu.

Todos los modelos ajustados tienen un acierto en la predicción superior al nivel basal (0.62, línea discontinua). El modelo SVMRadial  consigue el accuracy promedio más alto, seguido muy de cerca por logistic. Para determinar si las diferencias entre ellos son significativas, se recurre a test estadísticos. 
...
comparaciones
```
Acorde a las comparaciones por pares, existen evidencias suficientes para considerar que la capacidad predictiva de los modelos SVM radial y logistic es distinta p-value > 0.05 (0.1801641152).

Para predecir tendremos en cuenta el modelo SVM RAdial.

## Predicción y resultados

Consulta los parametros de la llamada a la funcion predict en este enlace: entre estos, la opcion
type merece ser estudiada. Con la opcion probs se calcula, por caso de test, la probabilidad a-posteriori para
cada valor de la clase; con la opcion raw nos quedamos, por caso de test, con el valor de la variable clase con
mayor probabilidad a-posteriori. A partir de esta segunda opcion podemos calcular la matriz de confusion y
la coleccion de estadsticos de evaluacion asociados; al n y al cabo esto supone \cruzar", por caso de test, los
valores clase predichos con los valores clase reales.

type raw https://www.rdocumentation.org/packages/caret/versions/6.0-72/topics/extractPrediction
...
SVM RAdial tiene su logica porqué es un SVM y estamos haciendo un problema de NLP.
