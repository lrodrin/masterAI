...
```{r}
set.seed(123)
cv.folds <- createMultiFolds(train.df$target, k=10, times=)
cv.cntrl <- trainControl(method = "repeatecv", number = 10,
...
- **number**. El número de hojas (*10*).
- **index**. Lista con los elementos para cada iteración (**cv.folds**). Cada elemento de la lista es un vector de números enteros correspondientes a las filas utilizadas para el entrenamiento en esa iteración. Vemos las 10 primeras hojas.
...
El *Accuracy* promedio estimado mediante validación cruzada repetida es de 0.78, el modelo predice correctamente un 78% de las veces.
...
Empleando un modelo Random Forest con mtry = 7 se consigue un *Accuracy* promedio de validación del 84%. 
...
Empleando un modelo Boosting con n.trees = 100, minobsinnode = 2, interaction.depth = 2 y shrinkage = 0.1 se consigue un *Accuracy* promedio de validación del 83%. 
