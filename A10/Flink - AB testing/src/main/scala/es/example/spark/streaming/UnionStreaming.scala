...
/**
  * Ejemplo de union de dos streams
  *
  * Uso: Main <hostname> <port>
  ...
  */
object UnionStreaming {
  def main(args: Array[String]): Unit = {
    if (args.length < 2) {
      System.err.println("uso: UnionStreaming <host> <puerto>")
      System.exit(1)
    }
    ...
    val lines = ssc.socketTextStream(args(0), args(1).toInt, StorageLevel.MEMORY_AND_DISK_SER).flatMap(_.split(" "))
    val linesUppercased = lines.map(_.toUpperCase)
    val allLines = lines.union(linesUppercased)

    val wordCounts = allLines.map(x => (x, 1)).reduceByKey(_ + _)
    wordCounts.print()
    ssc.start()
