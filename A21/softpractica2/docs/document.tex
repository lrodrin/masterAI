...
Con este comando hemos ejecutado durante 100 partidas al agente Pac-Man en el laberinto \textit{lab1}, que únicamente tiene un fantasma (ver Figura~\ref{lab1}). Durante la ejecución veremos que el agente encuentra rápidamente la mejor solución para resolver el problema a partir de la segunda partida (ver Figura~\ref{result_lab1}). La puntuación obtenida (\textit{Average Score}) por el agente es:
...
El agente Pac-Man ha ganado en todas las partidas (\textit{Win Rate: 100/100}). La puntuación media obtenida es positiva, concretamente su valor es igual a 191.86. La mejor puntuación obtenida en una partida es igual a 192. Para ganar una partida y obtener la mejor puntuación, el agente necesita realizar 9 movimientos. Estos movimientos son:
...
Volvemos a ejecutar al agente, pero esta vez en el laberinto \textit{lab2} que tiene dos fantasmas (ver Figura~\ref{lab2}). Lo hacemos con el siguiente comando:

\begin{lstlisting}[language=python, basicstyle=\footnotesize]
python busters.py -p RLAgent -k 2 -l lab2.lay -n 100
\end{lstlisting}

Durante esta ejecución también veremos como el agente encuentra rápidamente la mejor solución para resolver el problema a partir de la segunda partida (ver Figura~\ref{result_lab2}). La puntuación obtenida (\textit{Average Score}) por el agente es:

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{result_lab2}
	\caption{Puntuación obtenida por Pac-Man \\ en el laberinto número dos.}
	\label{result_lab2}
\end{figure}

El agente Pac-Man vuelve a ganar en todas las partidas (\textit{Win Rate: 100/100}). La puntuación media obtenida también es positiva, concretamente su valor es igual a 390.9. La mejor puntuación obtenida en una partida es igual a 391. Esta vez, para ganar una partida y obtener la mejor puntuación el agente necesita realizar 10 movimientos. Estos movimientos son:
...
Finalmente, ejecutamos el agente en el último de los laberintos disponibles, el laberinto \textit{lab3}, que tiene 3 fantasmas, una pared interior y un punto (ver Figura~\ref{lab3}). Para ello, usamos el siguiente comando:

\begin{lstlisting}[language=python, basicstyle=\footnotesize]
python busters.py -p RLAgent -k 3 -l lab3.lay -n 100
\end{lstlisting}

Durante esta ejecución también vemos como el agente rápidamente encuentra la mejor solución para resolver el problema. La puntuación obtenida (\textit{Average Score}) por el agente es:
...
En cada ejecución se tiene que inicializar la tabla Q a 0.
Comentar el primer caso, normalmente con la menor puntuación. el escalado de ir más rápido cada vez del enunciado.
