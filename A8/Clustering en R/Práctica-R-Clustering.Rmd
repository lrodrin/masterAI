...
Para encontrar el par de variables descriptoras a partir del cual se logrará un mejor agrupamiento, se usa la matriz de correlación.
...
Como se puede observar, las variables Cell.size y Cell.shape son las variables con el valor de correlación más elevado (0.9072282). Como consecuencia, sus elementos se encuentran más cercanos entre sí, pero con menos solapamiento. Así que, se usa este par de variables en la ejecución del algoritmo de clustering. Pero antes, comprobamos que estos pares de variables forman dos grupos claramente definidos.
...
Y se procede a la ejecución del algoritmo de clustering.
...
```{r}
#INSERTAR CÓDIGO AQUÍ
preProcess(BreastCancer.features, method = c("center", "scale"))
set.seed(101)
km_clusters <- kmeans(BreastCancer.features[c("Cell.size", "Cell.shape")], centers = 2, nstart = 20)
km_clusters
clusters <- km_clusters$cluster
...
```{r}
#INSERTAR CÓDIGO AQUÍ
preProcess(BreastCancer.features, method = c("center", "scale", "YeoJohnson"))
set.seed(101)
km_clusters <- kmeans(BreastCancer.features[c("Cell.size", "Cell.shape")], centers = 2, nstart = 20)
km_clusters
clusters <- km_clusters$cluster
...
```{r}
#INSERTAR CÓDIGO AQUÍ
preProcess(BreastCancer.features, method = c("range"))
set.seed(101)
km_clusters <- kmeans(BreastCancer.features[c("Cell.size", "Cell.shape")], centers = 2, nstart = 20)
km_clusters
clusters <- km_clusters$cluster
...
```{r}
#INSERTAR CÓDIGO AQUÍ
preProcess(BreastCancer.features, method = c("range", "YeoJohnson"))
set.seed(101)
km_clusters <- kmeans(BreastCancer.features[c("Cell.size", "Cell.shape")], centers = 2, nstart = 20)
km_clusters
clusters <- km_clusters$cluster
