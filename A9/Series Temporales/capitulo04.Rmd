...
```{r}
arima.1 <- arima(tsstationary, order = c(0, 1, 2))
arima.2 <- arima(tsstationary, order = c(1, 1, 0))
arima.3 <- arima(tsstationary, order = c(1, 1, 2))
arima.4 <- arima(tsstationary, order = c(1, 1, 1))
arima.5 <- arima(tsstationary, order = c(1, 1, 2))
arima.6 <- arima(tsstationary, order = c(0, 1, 1))
arima.7 <- arima(tsstationary, order = c(1, 1, 0))
```

Cálculo del valor *AIC* para seleccionar el modelo más optimizado y eficiente.

```{r}
AIC(arima.1, arima.2, arima.3, arima.4, arima.5, arima.6, arima.7)
```
...
```{r}
pred.ts <- forecast(arima.4, h = 10, level = c(99.5))
plot(pred.ts)
```
