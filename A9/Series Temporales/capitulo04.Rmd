...
Puesto que el entrenamiento del modelo de una serie temporal muy larga puede ser computacionalmente costoso, se filtran los datos de la serie para contemplar sólo desde el lunes 8 de junio hasta el domingo 14 de junio. Para no modificar el conjunto de datos al añadir otra estacionalidad usaremos la función *msts()*, que al contrario que la función *ts()* que ya hemos usado, puede manejar diferentes estacionalidades de una misma serie temporal.
...
En este caso, el proceso de ajuste será un proceso automático y necesitaremos ejecutar la función *auto.arima()* para encontrar el modelo más optimizado y eficiente.

```{r}
trainset <- subset(tsstationary, end = length(tsstationary) - (24 * 60 / 10))
testset <- subset(tsstationary, start = length(tsstationary) - (24 * 60 / 10) + 1)
```


```{r}
fitARIMA <- auto.arima(trainset, trace = TRUE, seasonal = FALSE)
fitARIMA
```
...
```{r}
forecast.arima <- forecast(fitARIMA, 24 * 60 / 10)
plot(forecast.arima)
autoplot(forecast.arima) + autolayer(testset)
```
