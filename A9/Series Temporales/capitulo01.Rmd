...
data$datetime <-
  parse_date_time(data$datetime, "dmY HMS", truncated = 3, tz = "UTC")
head(data, 10)
```
...
Observando la figura se puede detectar una dependencia estacional del consumo eléctrico, aunque pueden existir otros factores que pueden afectarla, como los días festivos, los fines de semana, ...

Parece que el principal desafío introducido por la serie temporal podría ser su estacionalidad múltiple. En ese caso, las siguientes estacionalidades pueden estar presentes:
...
<!-- Aun así, se tiene que eliminar la estacionalidad. Para ello usaremos la función *ndiffs()* para determinar el número de primeras diferencias necesarias para que la serie temporal no sean estacional, y la función *diff()* para eliminar la estacionalidad. -->

```{r eval=FALSE, include=FALSE}
ndiffs(ts) 
```
<!-- El número de primeras diferencias necesarias para que la serie temporal no sean estacional es 1. Como consecuencia el parámetro *differences* de la función *diff()* será igual a 1. -->
